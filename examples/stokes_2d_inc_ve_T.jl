...
@kernel inbounds = true function update_velocity!(V, r_V, Pr, τ, ρgy, η_ve, νdτ, g::StructuredGrid)
    I = @index(Global, NTuple)
    ...
    @time begin
        for it in 1:nt
            ...
            for iter in 1:niter
                ...
                update_velocity!(backend, 256, size(grid, Vertex()))(V, r_V, Pr, τ, ρgy, η_ve, νdτ, grid)
                bc!(arch, grid, bc_V...)
                if it > 1
