...
function compute(arch, grid, launch, backend, q, C, χ, Δt, nt)
    tic = time_ns()
    for _ in 1:nt
...
@views function main(backend=CPU(); nxy_l=(126, 126))
    ...
    launch_out = Launcher(arch, grid; outer_width=(32, 8))
    # visualisation
    ...
    for lau in (launch, launch_out)
        ...
        # warmup
        compute(arch, grid, lau, backend, q, C, χ, Δt, warmup)
        # time
        for ex in 1:5
            println("Experiment = $ex")
            wtime = compute(arch, grid, lau, backend, q, C, χ, Δt, (iters - warmup))
            # report
            ...
            round(T_eff, sigdigits=6), CUDA.device())
        end
    end
